...
export default function DestinationScreen({ navigation, route }) {
  ...
  function selectDestination(coord, label) {
    // Picking a location to save as a Home/Work favourite is a different
    // intent from booking — don't fire off to a booking screen for that.
    const wasSettingFav = !!settingFav;
        ...
        <View style={s.inputCard}>
          <TouchableOpacity style={[s.inputRow, s.pickupRow]} onPress={handlePickupTap} activeOpacity={0.7}>
            ...
            {gpsLoading ? (
              ...
              <Text style={s.inputTxt} numberOfLines={1} ellipsizeMode="tail">{pickupLabel}</Text>
            )}
          </TouchableOpacity>
          ...
          <View style={wp.modalSheet}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={wp.nowQuickSub}>Dispatch an ambulance right away</Text>
              </View>
